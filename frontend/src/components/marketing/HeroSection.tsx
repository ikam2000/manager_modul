...
export function HeroSection({
  ...
  return (
    <section className="mk-hero" style={{ padding: '128px 0 104px', background: 'var(--bg)' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1.08fr 0.92fr', gap: 56, alignItems: 'center', maxWidth: 1200, margin: '0 auto', padding: '0 32px' }}>
        <div>
          ...
          <h1 style={{ fontSize: 58, fontWeight: 700, letterSpacing: '-0.045em', lineHeight: 1.02, marginBottom: 22, color: 'var(--text)' }}>{headline}</h1>
          {subheadline && <p style={{ fontSize: 19, lineHeight: 1.72, color: 'var(--text-secondary)', marginBottom: 32, maxWidth: 620 }}>{subheadline}</p>}
          ...
          <div className="mk-stat-row">
            {[
              ...
              <div key={title} className="mk-metric-card" style={{ padding: 18 }}>
                ...
                <div style={{ fontSize: 13, lineHeight: 1.55, color: 'var(--text-secondary)' }}>{description}</div>
              </div>
            ))}
          </div>
        </div>
        <div className="mk-hero-panel" style={{ padding: 26 }}>
          <div style={{ padding: 18, borderRadius: 18, border: '1px solid rgba(255,255,255,0.09)', background: 'rgba(17,26,49,0.5)', marginBottom: 18 }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14 }}>
              {[
                ...
                <div key={title} style={{ padding: 14, borderRadius: 14, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
                  <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text)', marginBottom: 6 }}>{title}</div>
                  <div style={{ fontSize: 12, lineHeight: 1.55, color: 'var(--text-secondary)' }}>{desc}</div>
                </div>
              ))}
            </div>
          </div>
