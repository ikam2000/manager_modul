...
const MODULES = [
  { icon: <Database size={24} style={{ color: '#79a8ff' }} />, title: 'Каталог номенклатуры', description: 'SKU, категории, подкатегории, атрибуты, версии, спецификации, товарные номера и листы вопросов.' },
  { icon: <Users size={24} style={{ color: '#79a8ff' }} />, title: 'Поставщики и производители', description: 'Реквизиты, ИНН, DaData, связи с клиентами и производителями, договоры и приложения.' },
  { icon: <FileText size={24} style={{ color: '#79a8ff' }} />, title: 'Документы', description: 'PDF, JPEG, PNG, Excel, OCR, импорт номенклатуры и поставок, шаблоны и публичный просмотр.' },
  { icon: <QrCode size={24} style={{ color: '#79a8ff' }} />, title: 'QR-маркировка', description: 'Генерация кодов, печать, поиск по коду, макеты этикеток и публичные страницы сущностей.' },
  { icon: <Plug size={24} style={{ color: '#79a8ff' }} />, title: 'Интеграции', description: 'Shopify, Wildberries, Ozon, Excel, CSV, X-Api-Key, OAuth, callback-потоки и webhooks.' },
  { icon: <BarChart3 size={24} style={{ color: '#79a8ff' }} />, title: 'Trader и аналитика', description: 'Наценки поставщиков и категорий, dashboard, отчёты, фильтры, выгрузка и история изменений.' },
]
...
export default function Platform() {
  ...
  return (
    <div style={{ background: 'var(--bg)' }}>
      <Section style={{ padding: '120px 0 72px' }} background="default">
        ...
        <FeatureGrid columns={3}>
          {BANDS.map((band) => (
            <div key={band.title} className="mk-band-card" style={{ padding: 28 }}>
              ...
              <p style={{ fontSize: 15, lineHeight: 1.75, color: 'var(--text-secondary)', marginBottom: 12 }}>{band.description}</p>
              <ul className="mk-list">{band.bullets.map((b) => <li key={b}>{b}</li>)}</ul>
            </div>
          ))}
        </FeatureGrid>
        <div style={{ textAlign: 'center', marginTop: 44 }}>
          <a href={formHref('/request-demo.html', theme)} className="btn-mk-primary">Запросить демо</a>
