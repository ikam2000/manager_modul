  ...
  return (
    <div style={{ background: '#f8fafc' }}>
      <Section style={{ padding: '120px 0 64px' }} background="light">
        <SectionHeader label="Сценарии" title="Как ikamdocs используется в реальных процессах" description="Эта страница должна продавать решение через ситуации бизнеса: проблема, решение, эффект." />
        <div className="mk-use-case-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 28 }}>
          {CASES.map((item) => (
            <div key={item.title} className="mk-use-case-card" style={{ padding: 28 }}>
              ...
              <div style={{ fontSize: 15, lineHeight: 1.75, color: '#0f172a', marginBottom: 10 }}><strong>Проблема:</strong> {item.problem}</div>
              <div style={{ fontSize: 15, lineHeight: 1.75, color: '#0f172a', marginBottom: 10 }}><strong>Решение:</strong> {item.solution}</div>
              <div style={{ fontSize: 15, lineHeight: 1.75, color: '#0ea5e9' }}><strong>Эффект:</strong> {item.benefit}</div>
            </div>
          ))}
        </div>
      </Section>
